...
"ref":"msiempy",
"url":0,
"doc":"Welcome to the  msiempy library documentation. The pythonic way to deal with McAfee SIEM API.  Links : [GitHub](https: github.com/mfesiem/msiempy), [PyPI](https: pypi.org/project/msiempy/), [Class diagram](https: mfesiem.github.io/docs/msiempy/classes.png), [Packages diagram](https: mfesiem.github.io/docs/msiempy/packages.png), [SIEM API documentation](https: mfesiem.github.io) (generated PDFs)   Installation   python3 -m pip install msiempy    Authentication and configuration setup The module offers a single point of authentication against your SIEM, so you don't have to worry about authentication when writting your scripts. This means that you need to preconfigure the authentication using the configuration file. The configuration file is located (by default) securely in your user directory since it contains credentials. - For Windows:  %APPDATA%\\.msiem\\conf.ini - For Mac :  $HOME/.msiem/conf.ini - For Linux :  $XDG_CONFIG_HOME/.msiem/conf.ini or :  $HOME/.msiem/conf.ini   [esm] host = HOST user = USER passwd = PASSWORD's BASE64 [general] verbose = no quiet = no logfile = /var/log/msiempy/log.txt timeout = 60 ssl_verify = no   To set the password, you can use the [ msiempy_setup.py ](https: github.com/mfesiem/msiempy/blob/master/samples/msiempy_setup.py) script. You can also directly paste the password's base64 in the config file by doing:   >>> import base64 >>> passwd = 'P@assW0rd' >>> print(base64.b64encode(passwd.encode('utf-8' .decode( UEBhc3NXMHJk    Examples  Acknowledge alarms See objects:  msiempy.alarm.AlarmManager and  msiempy.alarm.Alarm Print all  unacknowledged alarms of the year who's name  match  'Test alarm' and triggering event message match  'Wordpress' . Then acknowledge the alarms and make sure they are all acknowledged. The number of alarms retreived is defined by the  page_size property.   from msiempy.alarm import AlarmManager, Alarm alarms=AlarmManager( time_range='CURRENT_YEAR', status_filter='unacknowledged',  This filter is computed on the server side filters=[('alarmName', 'Test alarm')],  Other filters are applied as regex event_filters=[('ruleName','Wordpress')], page_size=5)  Will only load 5 alarms (per query), increase limit to 500 or 1000 once finish testing for better performance print(\"Alarm list before load_data: \") print(alarms) print(alarms.json)  It's an empty list alarms.load_data()  Load the data into the list print(\"Alarm list: \") print(alarms)  -> \" ,  > containing 0 elements ; keys=set()\" print(alarms.get_text( fields=['id','triggeredDate','acknowledgedDate', 'alarmName', 'acknowledgedUsername']  -> Nice prettytable string print(\"Acknowledge alarms .\") [ alarm.acknowledge() for alarm in alarms ] while any( [ alarm['acknowledgedDate'] None for alarm in alarms ] ): time.sleep(1) [ alarm.refresh() for alarm in alarms ] print(alarms.get_text( fields=['id','triggeredDate','acknowledgedDate', 'alarmName', 'acknowledgedUsername']    Output    Alarm list before load_data:  ,  > containing 0 elements ; keys=set() [] INFO - Login into ESM 207.179.200.58:4443 with username NGCP. Last login 12/12/2019 18:04:48 INFO - Getting alarms infos . INFO - Getting events infos . INFO - The alarm Test Alarm (12/12/2019 18:05:26) has no events associated INFO - 4 alarms are matching your filter(s) Alarm list:  ,  > containing 4 elements ; keys={'alretRateMin', 'filters', 'NE', 'severity', 'XMIN', 'matchField', 'description', 'queryId', 'escalatedDate', 'alertRateCount', 'percentAbove', 'NID', 'offsetMinutes', 'acknowledgedDate', 'triggeredDate', 'percentBelow', 'EC', 'CTYPE', 'assignee', 'assigneeId', 'caseName', 'summary', 'DCHNG', 'caseId', 'id', 'alarmName', 'matchValue', 'actions', 'iocName', 'conditionType', 'iocId', 'acknowledgedUsername', 'useWatchlist', 'maximumConditionTriggerFrequency', 'events'} | id | triggeredDate | acknowledgedDate | alarmName | acknowledgedUsername | | {'value': 3840} | 12/12/2019 17:54:46 | None | Test Alarm | | | {'value': 3839} | 12/12/2019 17:42:56 | None | Test Alarm | | | {'value': 3838} | 12/12/2019 17:29:16 | None | Test Alarm | | | {'value': 3837} | 12/12/2019 17:11:56 | None | Test Alarm | | Acknowledge alarms . | id | triggeredDate | acknowledgedDate | alarmName | acknowledgedUsername | | {'value': 3840} | 12/12/2019 17:54:46 | 12/12/2019 18:07:53 | Test Alarm | | | {'value': 3839} | 12/12/2019 17:42:56 | 12/12/2019 18:07:53 | Test Alarm | | | {'value': 3838} | 12/12/2019 17:29:16 | 12/12/2019 18:07:53 | Test Alarm | | | {'value': 3837} | 12/12/2019 17:11:56 | 12/12/2019 18:07:53 | Test Alarm | |     Query events Query events according to destination IP and hostname filters, sorted by AlertID.   from msiempy.event import EventManager, FieldFilter print('Simple event query sorted by AlertID') events = EventManager( time_range='CURRENT_YEAR', fields=['SrcIP', 'AlertID'],  SrcIP and AlertID are not queried by default filters=[ FieldFilter('DstIP', ['0.0.0.0/0',]), FieldFilter('HostID', ['mail'], operator='CONTAINS')],  Please replace \"mail\" by a test hostname order= 'ASCENDING', 'AlertID' , limit=10)  Will only load 10 events (per query), increase limit to 500 or 1000 once finish testing for better performance events.load_data() print(events) print(events.get_text(fields=['AlertID','LastTime','SrcIP', 'Rule.msg']     Output   Simple event query sorted by AlertID INFO - Login into ESM 207.179.200.58:4443 with username NGCP. Last login 12/12/2019 18:18:12 WARNING - The query is not complete . Try to divide in more slots or increase the limit  ,  > containing 10 elements ; keys={'Alert.AlertID', 'Alert.SrcIP', 'Rule.msg', 'Alert.LastTime', 'Alert.IPSIDAlertID'} | AlertID | LastTime | SrcIP | Rule.msg | | 139345 | 12/09/2019 18:22:21 | 22.22.24.4 | Postfix Message moved to incoming queue | | 139346 | 12/09/2019 18:22:21 | 22.22.24.4 | Postfix Message moved to active queue for delivery | | 139351 | 12/09/2019 18:22:21 | 22.22.24.4 | Postfix Message sent | | 139352 | 12/09/2019 18:22:21 | 22.22.24.4 | Linux Postfix qmgr mail removed | | 139367 | 12/09/2019 18:19:09 | 22.22.24.4 | Postfix Max connection rate statistics | | 139368 | 12/09/2019 18:19:09 | 22.22.24.4 | Postfix Max connection count statistics | | 139369 | 12/09/2019 18:19:09 | 22.22.24.4 | Postfix Max cache size statistics | | 139370 | 12/09/2019 18:20:16 | 209.85.160.178 | Postfix Connect from host | | 139371 | 12/09/2019 18:20:17 | 209.85.160.178 | Postfix Client message transfer | | 139372 | 12/09/2019 18:20:17 | 209.85.160.178 | Postfix Disconnect from host |     Add a note to an events Setting the note of an event, retreiving the genuine event from IPSIDAlertID and checking the note is well set. See [ add_wpsan_note.py ](https: github.com/mfesiem/msiempy/blob/master/samples/add_wpsan_note.py) script to see more how to add note to event that triggered alarms !   from msiempy.event import EventManager, Event events = EventManager( time_range='CURRENT_YEAR', limit=2 ) events.load_data() for event in events : event.set_note(\"Test note\") genuine_event = Event(id=event['IPSIDAlertID'])  Event data will be loaded with ipsGetAlertData assert \"Test note\" in genuine_event['note'], \"The note doesn't seem to have been added to the event {}\".format(event)   See:  msiempy.core.query.FilteredQueryList ,  msiempy.event.EventManager ,  msiempy.event.FieldFilter ,  msiempy.event.Event  msiempy.event.EventManager() have other arguments:  order ,  start_time and  end_time or  time_rage  msiempy.event.EventManager.load_data() method accept also several parameters. It controls the query's division time range into slots of  delta duration, then the query would be divided into the specified number of  slots . Control also the number of asyncronous jobs using  workers parameter.  max_query_depth parameter specify the number of sub-divisions the query can take at most (zero by default). The query is divided only if it hasn't completed with the current query settings. See module documentation for more infos. See [ dump_all_fields.py ](https: github.com/mfesiem/msiempy/blob/master/samples/dump_all_fields.py) script to have full list of  fields you can request and fields you can use with  FieldFilter .  Print ESM infos Print a few esm infos. ESM object has not state for it self, it's a simple interface to data structures / values returned by the SIEM.   >>> from msiempy.device import ESM >>> esm=ESM() >>> esm.version() '11.2.1' >>> esm.recs() [('ERC-1', 144116287587483648)] >>> esm.buildstamp() '11.2.1 20190725050014'   See:  msiempy.device.ESM  Dump Datasources Load all datasources and write all infos in a CSV file.   from msiempy.device import DevTree devtree = DevTree() with open('all-datasources.csv', 'w') as f: f.write(devtree.get_text(format='csv'   See:  msiempy.device.DevTree ,  msiempy.device.DataSource  Dump Watchlists definitions Print whatchlist list.   from msiempy.watchlist import WatchlistManager watchlists=WatchlistManager() print(watchlists)   See:  msiempy.watchlist.WatchlistManager ,  msiempy.watchlist.Watchlist  Use the Session object You can choose not to use wrapper ojects like  AlarmManager and use directly the Session object to make any API calls with any data. The Session object will handle intermittent SIEM errors.   from msiempy import NitroSession s = NitroSession() s.login()  Get all last 24h alarms details with ESM API v2 (not supported by other objects yet) alarms = s.api_request('v2/alarmGetTriggeredAlarms?triggeredTimeRange=LAST_24_HOURS&status=&pageSize=500&pageNumber=1') for a in alarms: a.update(s.api_request('v2/notifyGetTriggeredNotificationDetail', {'id':a['id']}   See  msiempy.core.session.NitroSession  And more . See the [samples folder](https: github.com/mfesiem/msiempy/tree/master/samples) for more detailed uses ! You can also review the [tests](https: github.com/mfesiem/msiempy/tree/master/tests).  Contribute Pull requests are welcome! Please read the [contributing](https: github.com/mfesiem/msiempy/blob/master/CONTRIBUTING.md) file.  Run tests Run all tests, sometimes the tests have to be reruns.   pytest  reruns 3   You might also want to run per-file tests   pytest tests/auth/test_device.py   Or per-method test   python3 -m unittest tests.auth.test_event.T.test_add_note    Code analysis [![Codacy Badge](https: app.codacy.com/project/badge/Grade/114821fcf6e14b8eb0f927e0112488c8)](https: www.codacy.com/gh/mfesiem/msiempy?utm_source=github.com&amp;utm_medium=referral&amp;utm_content=mfesiem/msiempy&amp;utm_campaign=Badge_Grade) [![Maintainability](https: api.codeclimate.com/v1/badges/0cc21ba8f82394cb05f3/maintainability)](https: codeclimate.com/github/mfesiem/msiempy/maintainability)  Error report Configure log file reporting in the configuration file and and look for  \"ERROR\" . Useful shell command to get simple list of errors:   cat /path/to/your/log/file | grep -i error | sort | uniq  "
},
{
"ref":"msiempy.event",
"url":1,
...
"url":1,
"doc":"Get the event ID. Try to return  e['Alert.IPSIDAlertID']  or concatenate  e['ipsId']['id'] and  e['alertId'] depending if Event is loaded from the SIEM  qryGetResults api call or  ipsGetAlertData ",
"func":1
},
{
...
"url":9,
"doc":"Dict-Like object. Common keys : -  id : The ID of the triggered alarm -  summary : The summary of the triggered alarm -  assignee : The assignee for this triggered alarm -  severity : The severity for this triggered alarm -  triggeredDate : The date this alarm was triggered -  acknowledgedDate : The date this triggered alarm was acknowledged -  acknowledgedUsername : The user that acknowledged this triggered alarm -  alarmName : The name of the alarm that was triggered -  events : The events that triggered the alarm - And others . see  msiempy.alarm.Alarm.ALARM_FIELDS_MAP Arguments: -  adict : Alarm parameters -  id : The alarm ID to instanciate. Will load informations Creates a empty Alarm."
},
{
...
"url":9,
"doc":"Retreive the genuine Event object from an Alarm. Warning : This method will load the details of the first triggering event only. Arguments: -  use_query : Uses the query module to retreive common event data. Only works with SIEM v 11.2 or greater. Default behaviour will call  ipsGetAlertData to retreive the complete event definition. -  extra_fields : Only when  use_query=True . Additionnal event fields to load in the query. See :  msiempy.event.EventManager ",
"func":1
},
{
"ref":"msiempy.alarm.Alarm.ALARM_FIELDS_MAP",
"url":9,
"doc":"List of all  Alarm possible fields. This mapping is used to create new alarms and change genuine (UPPERCASE) key names to explicit ones. If the value is None, the original key name is kept, otherwise the UPPERCASE key won't work.   {'EC': None, 'SMRY': 'summary', 'NAME': 'alarmName', 'FILTERS': 'filters', 'CTYPE': None, 'QID': 'queryId', 'ARM': 'alretRateMin', 'ARC': 'alertRateCount', 'PCTA': 'percentAbove', 'PCTB': 'percentBelow', 'OFFSETMIN': 'offsetMinutes', 'TIMEF': 'maximumConditionTriggerFrequency', 'XMIN': None, 'USEW': 'useWatchlist', 'MFLD': 'matchField', 'MVAL': 'matchValue', 'SVRTY': 'severity', 'ASNID': 'assigneeId', 'ASNNAME': 'assignee', 'TRGDATE': 'triggeredDate', 'ACKDATE': 'acknowledgedDate', 'ESCDATE': 'escalatedDate', 'CASEID': 'caseId', 'CASENAME': 'caseName', 'IOCNAME': 'iocName', 'IOCID': 'iocId', 'DESC': 'description', 'NID': None, 'ACTIONS': 'actions', 'NE': None, 'EVENTS': 'events', 'DCHNG': None}  "
},
{
...
"url":9,
"doc":"Gets the alarm parameters based on an ID. Will still use private method if SIEM if v10.X because it's the only way to get the trigerring event ID.",
"func":1
},
{

...
"url":2,
"doc":"List items keys. Every items should have the same set of keys.",
"func":1
},
{
"ref":"msiempy.watchlist.WatchlistManager.get_text",
"url":2,
"doc":"Return a csv or table string representation of the list Arguments: -  format : prettytable: Returns a table generated by prettytable use MSWORD_FRIENDLY format. csv: Returns data with header and comma separated values. -  fields : list of fields you want in the table. If  None : default fields are returned by .keys attribute and sorted. -  max_column_width : when using prettytable only -  get_text_nest_attr : attributes passed to the nested  msiempy.core.types.NitroList.get_text elements if any. Useful to control events appearence.",
"func":1
},
{
...
{
"ref":"msiempy.event.EventManager.fields",
"url":3,
"doc":"List of query fields"
},
{
"ref":"msiempy.event.EventManager.limit",
"url":3,
"doc":"Max number of rows per query"
},
{
...
"url":3,
"doc":"Re-load event's data. Arguments: -  use_query ( bool ): Force the use of the query module to retreive the event data. In contrario, if explicitely  False , force the use of  ipsGetAlertData to get the details. The default behaviour will figure out the event type/load method based on the existence if 'Alert.IPSIDAlertID' keys -  extra_fields ( list ): Only when  use_query=True or the Event is already a query event. Additionnal event fields to load in the query.  Warning Enforce  use_query=True will reset the Events fields to whatever is passed to  extra_fields Raise  AttributeError if the event ID has not been found.",
"func":1
},
{
...
"url":5,
"doc":"Maximum number of alarms per query"
},
{
...
"url":9,
"doc":"SIEM API methos/parameters mapping. This structure provide a central place to aggregate API methods and parameters. Usage:  NitroSession.PARAMS.get(\"method\") Returns: -  tuple  : ( str or  Template ,  str or  Template ) : The first item is the SIEM API endpoint name. The second item is the JSON string data parameters required for the enpoint call. If the string is  Template string, it needs to be interpolate with paramaters. See  msiempy.core.session.NitroSession.request for a list of all possible calls."
},
{
